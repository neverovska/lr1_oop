        ...
        transferButton.setOnAction(event -> {
            try {
                ...
                if (buf1.getCount() >= Double.parseDouble(countText.getText()) && !buf.getBloking() && !buf.getFreezing()) {
                    ...
                    for(Bank_Account ba : out_client.getAccounts())
                    ...
                    Log log = new Log("-t-",buf1.getNumber(), Controller.bank.getBik(), buf.getNumber(), Double.parseDouble(countText.getText()));
                    Controller.bank.getLogs().add(log);
